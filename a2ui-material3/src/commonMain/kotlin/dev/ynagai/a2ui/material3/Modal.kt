 ...
 * is the same escape hatch `Icon` offers. A renderer arriving as a modal's content once segfaulted
 * Kotlin/Native when it was built on a Material 3 `Surface`, and the `Dialog` did not prevent it;
 * what does is the key on each component's `Render` call, which `ModalScrollSwapTest` holds on
 * this path (#31).
 *
