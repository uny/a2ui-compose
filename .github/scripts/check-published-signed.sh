...
# portable to every `sh`, so inside `find … | wc -l | tr -d ' '` the status is `tr`'s and find's
# exit on a missing directory is discarded. Both ways of looking at nothing -- an absent tree and
# a present but empty one -- are caught by the count.
...
# its sibling. `-s`, not `-f`: an empty `.asc` is a name, not a signature.
unsigned=$(find "$repo" -type f \
  ! -name '*.asc' ! -name '*.md5' ! -name '*.sha1' ! -name '*.sha256' ! -name '*.sha512' \
  ! -name 'maven-metadata*' \
  -exec sh -c '[ -s "$1.asc" ] || echo "$1"' _ {} \;)
if [ -n "$unsigned" ]; then
