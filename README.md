...
| **`org.jetbrains:markdown` 0.7.14** (`a2ui-material3-markdown` only) | `requires: 0.7.14` on `org.jetbrains:markdown` in that module's Gradle module metadata; it is `implementation`, so the parser is on your runtime class path and not your compile class path | **Silently**, the same way as Compose: a project that already depends on an older `org.jetbrains:markdown` is moved up to 0.7.14. The other three modules carry no parser and are unaffected. |
